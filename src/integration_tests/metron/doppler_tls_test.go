...
var _ = FDescribe("communicating with doppler over TLS", func() {
	const (
	...
	It("forwards messages", func() {
		...
		go func() {
			...
			Eventually(func() ([]byte, error) {
				c, _, err := websocket.DefaultDialer.Dial(fmt.Sprintf("ws://localhost:%d/apps/test-app-id/stream", dopplerOutgoingPort), nil)
